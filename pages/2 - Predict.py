...
# st.write(classification_report(y_test, y_pred, target_names=y_test.unique()))
...
island = st.selectbox(
   "Island",
   X.island.unique(),
   index=None, # Also have the option to give some index number to choose default island
   placeholder="Choose island where penguin resides",
)

bill_length_mm = st.slider("Bill length", 0, 100, 20)
